#!/usr/bin/env python3
"""
Test script for Atomate2 and OPTIMADE functionality
This script checks if your environment is properly set up for materials science workflows.
"""
...
def test_optimade_queries() -> bool:
    ...
    try:
        ...
        filter_oqmd   = '_oqmd_stability<=0 AND elements HAS "O" AND nelements=3 AND _oqmd_band_gap>0'
        filter_alex   = '_alexandria_hull_distance<=0 AND elements HAS "O" AND nelements=3 AND _alexandria_band_gap>0'
        #filter_alex   = 'elements HAS "Pb" AND nelements=1'

        response_oqmd = 'id,_oqmd_entry_id,lattice_vectors,cartesian_site_positions,species_at_sites,_oqmd_band_gap'
        response_alex = 'id,chemical_formula_descriptive,lattice_vectors,cartesian_site_positions,species_at_sites,_alexandria_band_gap'
        ...
        oqmd_optimade_query = rest_base_oqmd + "&".join([filter_oqmd, response_oqmd]+page_)
        print("Created Query: \n\n{}".format(oqmd_optimade_query))
        alexandria_optimade_query = rest_base_alex + "&".join([filter_alex, response_alex]+page_)
        ...
        print("Fetched {} pages from OQMD".format(len(dataset)))
        print("\n\n")
        for i in range(1):
            ...
            if jsondata is None:
                ...
                query = deepcopy(jsondata['links']['next'])
                dataset.append(deepcopy(jsondata))
        print("Fetched {} pages from Alexandria".format(len(dataset)))
